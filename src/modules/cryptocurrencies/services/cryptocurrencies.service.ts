...
export class CryptocurrenciesService {
  ...
  async create(createCryptoDto: CreateCryptocurrencyDto): Promise<Cryptocurrency> {
    // Verificar que la moneda exista usando el servicio de currencies
    const currency = await this.currenciesService.findOne(createCryptoDto.currencyId);
  ...
  async update(
    ...
    if (updateCryptoDto.currencyId) {
      const currency = await this.currenciesService.findOne(updateCryptoDto.currencyId);
